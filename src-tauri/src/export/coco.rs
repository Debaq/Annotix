...
use super::{add_image_to_zip, parse_bbox, parse_keypoints, parse_obb, parse_polygon};
...
pub fn export<F: Fn(f64)>(
    ...
    for (i, image) in images.iter().enumerate() {
        ...
        for ann in &image.annotations {
            if let Some(coco_ann) =
                convert_annotation(ann, image_id, annotation_id, image.width, image.height)
            {
    ...
    match ann.annotation_type.as_str() {
        ...
        "obb" => {
            ...
            let (min_x, min_y, max_x, max_y) =
                obb_to_aabbox(obb.x, obb.y, obb.width, obb.height, obb.rotation);
            base["bbox"] = json!([min_x, min_y, max_x - min_x, max_y - min_y]);
            base["area"] = json!((max_x - min_x) * (max_y - min_y));
            Some(base)
        }
