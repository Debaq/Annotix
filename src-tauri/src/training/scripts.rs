...
pub fn generate_mmdet_config(req: &TrainingRequest, sp: &ScriptPaths) -> String {
    let bp = &req.backend_params;
...
"#,
        model_id = model_id,
        root = sp.root_py,
        ann_train = sp.rel(keys::ANN_TRAIN),
        ann_val = sp.rel(keys::ANN_VAL),
        images_train = sp.rel(keys::IMAGES_TRAIN),
        images_val = sp.rel(keys::IMAGES_VAL),
        batch = req.batch_size,
        workers = req.workers,
...
///
/// Devuelve pares `(nombre de archivo, contenido)`: los backends con archivo de
/// config aparte generan dos, el resto uno.
pub fn generate_train_script_for_backend(
    ...
    let with_config = |config_name: &str, config: String, train: String| {
        Ok(vec![
            (config_name.to_string(), config),
            ("train.py".to_string(), train),
        ])
    };
    let config_path = |name: &str| format!("{}/{}", ds.root_py(), name);
    ...
    match req.backend {
        ...
        TrainingBackend::MmDetection => {
            let sp = ds.script_paths(COCO)?;
            let path = config_path("mmdet_config.py");
            with_config(
                "mmdet_config.py",
                generate_mmdet_config(req, &sp),
                generate_mmdet_train_script(&path),
            )
        }
        TrainingBackend::Smp => {
        ...
        TrainingBackend::MmSegmentation => {
            let sp = ds.script_paths(MASKS)?;
            let path = config_path("mmseg_config.py");
            with_config(
                "mmseg_config.py",
                generate_mmseg_config(req, &sp),
                generate_mmseg_train_script(&path),
            )
        }
        TrainingBackend::Detectron2 => {
            let sp = ds.script_paths(COCO)?;
            one("train.py", generate_detectron2_script(req, &sp))
        }
        TrainingBackend::MmPose => {
            let sp = ds.script_paths(COCO)?;
            let path = config_path("mmpose_config.py");
            with_config(
                "mmpose_config.py",
                generate_mmpose_config(req, &sp),
                generate_mmpose_train_script(&path),
            )
        }
        TrainingBackend::MmRotate => {
            let sp = ds.script_paths(COCO)?;
            let path = config_path("mmrotate_config.py");
            with_config(
                "mmrotate_config.py",
                generate_mmrotate_config(req, &sp),
                generate_mmrotate_train_script(&path),
            )
        }
...
pub fn generate_mmseg_config(req: &TrainingRequest, sp: &ScriptPaths) -> String {
    let bp = &req.backend_params;
    let model_id = &req.model_id;
    let optimizer_type = bp
        .get("optimizer_type")
    ...
    let num_classes_with_bg = sp.num_classes + 1;
...
train_dataloader = dict(
    ...
    dataset=dict(
        type=dataset_type,
        data_root=data_root,
        data_prefix=dict(img_path="{images_train}", seg_map_path="{masks_train}"),
...
"#,
        ...
        images_train = sp.rel(keys::IMAGES_TRAIN),
        images_val = sp.rel(keys::IMAGES_VAL),
        masks_train = sp.rel(keys::MASKS_TRAIN),
        masks_val = sp.rel(keys::MASKS_VAL),
        crop_size = crop_size,
        batch = req.batch_size,
        workers = req.workers,
        optimizer_type = optimizer_type,
...
pub fn generate_detectron2_script(req: &TrainingRequest, sp: &ScriptPaths) -> String {
    let bp = &req.backend_params;
...
def main():
    ...
    # Register datasets
    train_json = ANN_TRAIN
    val_json = ANN_VAL
    train_imgs = IMAGES_TRAIN
    val_imgs = IMAGES_VAL
...
pub fn generate_mmpose_config(req: &TrainingRequest, sp: &ScriptPaths) -> String {
    let bp = &req.backend_params;
    let model_id = &req.model_id;
    let optimizer_type = bp
        .get("optimizer_type")
...
val_evaluator = dict(type="CocoMetric", ann_file=data_root + "/{ann_val}")
...
pub fn generate_mmrotate_config(req: &TrainingRequest, sp: &ScriptPaths) -> String {
    let bp = &req.backend_params;
    let model_id = &req.model_id;
    let optimizer_type = bp
        .get("optimizer_type")
...
def main():
    ...
    # Serie declarada por el preparador (ver training/contract.rs)
    ts = np.load(X_TRAIN).squeeze()
