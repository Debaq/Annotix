...
use super::notebook;
use super::scripts;
use super::{TrainingBackend, TrainingRequest};
...
pub fn generate_training_package(
    ...
    let zip_file =
        std::fs::File::create(zip_path).map_err(|e| format!("Error creando ZIP: {}", e))?;
...
fn add_dir_to_zip(
    ...
    for entry in std::fs::read_dir(dir).map_err(|e| e.to_string())? {
        ...
        if path.is_dir() {
            ...
            let data =
                std::fs::read(&path).map_err(|e| format!("Error reading file {}: {}", name, e))?;
            zip.write_all(&data)
